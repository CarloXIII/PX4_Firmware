...
int twist_angle_control_thread_main(int argc, char *argv[]) {
	...
	while (!thread_should_exit) {
		...
		/* get a local copy */
		if (rel_ang_updated)
			orb_copy(ORB_ID(vehicle_paraglider_angle), rel_ang_sub,
					&angle_measurement);
...
int twist_angle_control_main(int argc, char *argv[]) {
	...
	if (!strcmp(argv[1], "start")) {
		...
		deamon_task = task_spawn_cmd("twist_angle_control", SCHED_DEFAULT,
				SCHED_PRIORITY_MAX - 20, 2048, twist_angle_control_thread_main,
